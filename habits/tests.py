...
class HabitViewSetTestCase(APITestCase):
    ...
    def test_update_habit_for_user1(self):
        """Тест на обновление привычки для пользователя test_user1"""
        self.client.force_authenticate(user=self.test_user1)
        self.client.post("/habits/", self.habit_data, format="json")
    ...
    def test_delete_habit_for_user1(self):
        """Тест на удаление привычки для пользователя test_user1"""
        self.client.force_authenticate(user=self.test_user1)
        self.client.post("/habits/", self.habit_data, format="json")
    ...
    def test_user2_cannot_access_user1_data(self):
        """Тест, что test_user2 не может получить доступ к данным test_user1"""
        self.client.force_authenticate(user=self.test_user1)
        self.client.post("/habits/", self.habit_data, format="json")
